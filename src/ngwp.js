...
.option('-c, --config', 'Set module config (Default path/to/config/config.js)')
.option('-d, --dist <filename>', 'Set destination file')
.option('-b, --base <folder>', 'Set destination base path')
...
.action((mode) => {
  if (_.indexOf(['dev', 'develop', 'development'], mode) !== -1) {
    runDevelopTasks()
  } else if (_.indexOf(['prod', 'product', 'production'], mode) !== -1) {
    runReleaseTasks()
  } else if (_.indexOf(['test', 'unitest'], mode) !== -1) {
    runUnitestTasks()
  } else {
    runCommander.help()
